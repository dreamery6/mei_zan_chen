...
var sendMail = function (data) {
  logger.error("Johnny enter the send mail");
  //if (config.debug) {
  //  return;
  //}

  logger.error("Johnny enter the send mail " + JSON.stringify(config.mail_opts));
  // 遍历邮件数组，发送每一封邮件，如果有发送失败的，就再压入数组，同时触发mailEvent事件
  transporter.sendMail(data, function (err) {
    if (err) {
      // 写为日志
      logger.error("Johnny send mail failed!");
      logger.error(err);
    }
    else{
      logger.debug("Johnny send mail success!");
    }
  });
};
...
exports.sendActiveMail = function (who, token, name) {
  logger.error("sendActiveMail Send Email");
