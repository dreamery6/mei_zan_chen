 ...
 */
exports.CreateDatabase = function(pool, TableName, fields, callback){
    this.CreateDatabaseWithDBName(pool, config.mysql_database, TableName, fields, callback);
};
 ...
 * @param {Function} callback 回调函数
 */
exports.CreateDatabaseWithDBName = function(pool, Database, TableName, fields, callback){
    ...
    pool.getConnection(function (err, conn) {
        ...
        conn.query(createSQL,function(err,rows){
            if (err) {
                ...
                logger.error("error sql ==> " + createSQL);
                callback('failed');
            }
            else{
                callback('success');
...
function JoinWhereString(whereData){
    ...
    if(whereData){
        ...
        for(var key in whereData){
            ...
            if(key == "extra_where_string"){
                whereString += " and " + whereData[key];
            } else if(key == 'or' && typeof(whereData[key]) == 'object'){      //或者 OR
                whereString += " and (1=0 ";
                ...
                if(whereData[key].findInSet){
                    whereString += " and " + "FIND_IN_SET (" + whereData[key].findInSet + "," + key +")";
                }
 ...
 * @param {Function} callback 回调函数
 **/
exports.SelectDataCommon = function(pool, DBName, TableName, fields, whereData, opt, callback){
    SelectDataCommonImpl(pool, DBName, TableName, fields, whereData, opt, callback);
};
 ...
 * @param {Function} callback 回调函数
 **/
function SelectDataCommonImpl(pool, DBName, TableName, fields, whereData, opt, callback){
    if(!fields || fields.length <= 0){
        fields = "*";
    }
    var removeSpace = fields.replace(/\s*/g, "");
    var removePoint = removeSpace.replace(/\`/g, "");
    var field_list = removePoint.split(",");

    var join="";
    for(var key in field_list){       
        join +="`" + field_list[key]+ "`" + ",";       
    }
    fields = join.substring(0,join.lastIndexOf(','));
 ...
 * @param {Function} callback 回调函数
 **/
exports.GetCount = function(pool, DBName, TableName, IDFieldName, whereData, callback){
 ...
 * @param {Function} callback 回调函数
 **/
exports.GetSumAdvance = function(pool, DBName, TableName, MixFunction, whereData, callback){
 ...
 * @param {Function} callback 回调函数
 **/
exports.GetSum = function(pool, DBName, TableName, fieldName, whereData, callback){
 ...
 * @param {Function} callback 回调函数
 */
exports.InsertDataListCommon = function(pool, DBName, TableName, dataList, fieldList, callback){
 ...
 * @param {Function} callback 回调函数
 **/
exports.InsertDataCommon = function(pool, DBName, TableName, insertData, callback){
    if(!insertData){
 ...
 * @param {Function} callback 回调函数
 **/
exports.InsertDataCommonReturnID = function(pool, DBName, TableName, IDFieldName, insertData, callback){
    this.InsertDataCommon(pool, DBName, TableName, insertData, function(res){
        if(res != "success"){
            callback(-1);
            return;
        }
        SelectDataCommonImpl(pool, DBName, TableName, IDFieldName, insertData, null, function(rows){
            if(rows.length <= 0){
                callback(-1);
            }
            else{
...
function JoinUpdateData(updateData){
    if(!updateData){
        return;
    }
    var updateSQL = "";
    ...
    if(updateDataCount <= 0){
        logger.error("JoinUpdateData : No updateData recv!");
        return;
    }
 ...
 * @param {Function} callback 回调函数
 **/
exports.UpdateDataCommon = function(pool, DBName, TableName, updateData, whereData, callback){
    if(!updateData){
    ...
    var updateDataSql = JoinUpdateData(updateData);

    if(!updateDataSql || updateDataSql.length <= 0){
        logger.error("UpdateDataCommon : No updateData recv!");
        callback("success");
        return;
    }

    updateSQL += updateDataSql;
    ...
    pool.getConnection(function (err, conn) {
        ...
        conn.query(updateSQL,function(err1,rows1){
            if(err1){
                ...
                logger.debug("UpdateDataCommon " + TableName + " : success! -- " + JSON.stringify(rows1));
 ...
 * @param {object} updateData 数据列表
 * @param {object} whereData 更新数据的条件
 * @param {string} opt 额外的信息
 * @param {Function} callback 回调函数
 **/
exports.UpdateDataCommonExt = function(pool, DBName, TableName, updateData, whereData, opt, callback){
    if(!updateData){
        logger.error("UpdateDataCommonExt : NULL updateData recv!");
        callback("success");
        return;
    }
    ...
    if(!updateDataSql || updateDataSql.length <= 0){
        logger.error("UpdateDataCommonExt : No updateData recv!");
        callback("success");
        return;
    }
    ...
    pool.getConnection(function (err, conn) {
        if (err){
            logger.error("UpdateDataCommonExt " + TableName + " Pool Error : " + err);
            callback("failed");
            return;
        }
        ...
        conn.query(updateSQL,function(err1,rows1){
            if(err1){
                logger.error("UpdateDataCommonExt " + TableName + " Update Error : " + err1);
                callback("failed");
            }
            else{
                logger.debug("UpdateDataCommonExt " + TableName + " : success!");
                if(rows1 && rows1.affectedRows > 0){
                    callback("success");
                }else{
                    callback("failed");
 ...
 * @param {Function} callback 回调函数
 **/
exports.DeleteCommon = function(pool, DBName, TableName, whereData, callback){
 ...
 * @param {Function} callback 回调函数
 */
exports.EXETransaction = function(pool, queryList, callback){
    if(!queryList){
 ...
 * @param {Function} callback 回调函数
 **/
exports.ExeCommon = function(pool, sql, callback){
    pool.getConnection(function (err, conn) {
        if (err){
 ...
 * @param {Function} callback 回调函数
 **/
exports.SelectSQLCommon = function(pool, sql, callback){
    logger.debug(sql);
